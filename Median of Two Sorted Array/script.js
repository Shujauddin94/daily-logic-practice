...
function findMedianSortedArrays(nums1, nums2) {
    let merged = []; // We create an empty array called merged to store the merged elements from both nums1 and nums2. This array will be used to combine the two sorted arrays into a single sorted array, which will allow us to easily find the median.
    let i = 0, j = 0; // We initialize two pointers, i and j, to keep track of our current position in nums1 and nums2, respectively. We also initialize an empty array called merged to store the merged elements from both arrays.
...
function findMedianSortedArrays(nums1, nums2) { // We define a function called findMedianSortedArrays that takes two sorted arrays, nums1 and nums2, as input and returns the median of the combined sorted array.
  ...
  let low = 0; // We initialize low to 0 and high to the length of nums1, which represents the range of possible partition indices for nums1. We will use binary search to find the correct partition index for nums1.
  ...
  while (low <= high) { // We start a while loop that continues as long as low is less than or equal to high. This loop will allow us to perform binary search on nums1 to find the correct partition index.
    ...
    if (maxLeftX <= minRightY && maxLeftY <= minRightX) { // We check if we have found the correct partition by comparing the maximum of the left halves with the minimum of the right halves. If maxLeftX is less than or equal to minRightY and maxLeftY is less than or equal to minRightX, it means we have found the correct partition.
      if ((x + y) % 2 === 0) {
        return (
          Math.max(maxLeftX, maxLeftY) + // If the total length of the combined arrays is even, the median is the average of the maximum of the left halves and the minimum of the right halves. We use Math.max to find the maximum of the left halves and Math.min to find the minimum of the right halves. Finally, we divide the sum by 2 to get the average, which is the median when the total length is even.
        ...
        ) / 2;   // We calculate the median by taking the average of the maximum of the left halves and the minimum of the right halves. We use Math.max to find the maximum of the left halves and Math.min to find the minimum of the right halves. Finally, we divide the sum by 2 to get the average, which is the median when the total length is even.
      } else { // If total length of the combined arrays is odd, the median is simply the maximum of the left halves, since there will be one more element in the left half than in the right half. We use Math.max to find the maximum of the left halves and return it as the median. x + y is odd, so the median is the maximum of the left halves. We use Math.max to find the maximum of the left halves and return it as the median.
        return Math.max(maxLeftX, maxLeftY); // If the total length of the combined arrays is odd, the median is simply the maximum of the left halves, since there will be one more element in the left half than in the right half. We use Math.max to find the maximum of the left halves and return it as the median.
      }
    }
    // Move left
