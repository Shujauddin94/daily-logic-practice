...
function findMedianSortedArrays(nums1, nums2) {
    ...
    let len = merged.length; // We calculate the total length of the merged array to determine how to find the median.
    if (len % 2 === 0) {
...
function findMedianSortedArrays(nums1, nums2) {
  ...
  while (low <= high) {
    ...
    if (maxLeftX <= minRightY && maxLeftY <= minRightX) { // If the maximum of the left halves is less than or equal to the minimum of the right halves for both arrays, we have found the correct partition.
      if ((x + y) % 2 === 0) {
        return (
    ...
    // Move right
      ...
      low = partitionX + 1; // If the maximum of the left half of nums1 is greater than the minimum of the right half of nums2, it means we need to move the partition in nums1 to the left, so we update high to partitionX - 1.
    }
  }
}
console.log(findMedianSortedArrays([1, 3], [2])); // Output: 2.0
